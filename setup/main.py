...
class Setup(object):
    ...
    def __config_get_value(self, section, name, defval):
        def from_variant(v):
            if not isinstance(v, GLib.Variant):
                return v
            vt = v.classify()
            if vt == GLib.VariantClass.STRING or vt == GLib.VariantClass.OBJECT_PATH:
                return v.get_string()
            if vt == GLib.VariantClass.INT64:
                return v.get_int64()
            if vt == GLib.VariantClass.INT32:
                return v.get_int32()
            if vt == GLib.VariantClass.INT16:
                return v.get_int16()
            if vt == GLib.VariantClass.UINT64:
                return v.get_uint64()
            if vt == GLib.VariantClass.UINT32:
                return v.get_uint32()
            if vt == GLib.VariantClass.UINT16:
                return v.get_uint16()
            if vt == GLib.VariantClass.DOUBLE:
                return v.get_double()
            if vt == GLib.VariantClass.BYTE:
                return v.get_byte()
            if vt == GLib.VariantClass.BOOLEAN:
                return v.get_boolean()
            if vt == GLib.VariantClass.VARIANT:
                return from_variant(v.get_variant())
            if vt == GLib.VariantClass.ARRAY or vt == GLib.VariantClass.TUPLE:
                array = []
                for cv in v:
                    array.append(from_variant(cv))
                return array
            if vt == GLib.VariantClass.DICT_ENTRY:
                return (from_variant(v.get_child_value(0)), from_variant(v.get_child_value(1)))
            return None
        try:
            retval = self.__config.get_value(section, name)
            retval = from_variant(retval)
        except:
            retval = None
        if retval == None:
            self.__config_set_value(section, name, defval)
            return defval
        return retval

    def __config_set_value(self, section, name, val):
        def to_variant(v):
            ...
            if isinstance(v, list) or isinstance(v, tuple):
                array = map(to_variant, v)
                return GLib.Variant("av", array)
            return None
        var = to_variant(val)
        if var != None:
            self.__config.set_value(section, name, to_variant(val))
        else:
            print >> sys.stderr, "set_value(%s,%s,%s) failed" % (section, name, repr(val))
    ...
    def __init_ui(self):
        ...
        shortcuts = self.__config_get_value(
        ...
        self.__combobox_lookup_table_orientation.set_active(
            self.__config_get_value("panel", "lookup_table_orientation", 0))
        ...
        self.__combobox_panel_show.set_active(
            self.__config_get_value("panel", "show", 0))
        ...
        self.__combobox_panel_position.set_active(
            self.__config_get_value("panel", "position", 3))
        ...
        self.__checkbutton_custom_font.set_active(
            self.__config_get_value("panel", "use_custom_font", False))
        ...
        if self.__config_get_value("panel", "use_custom_font", False):
        ...
        font_name = self.__config_get_value("panel", "custom_font", font_name)
        ...
        self.__checkbutton_show_icon_on_systray.set_active(
            self.__config_get_value("panel", "show_icon_on_systray", True))
        ...
        self.__checkbutton_show_im_name.set_active(
            self.__config_get_value("panel", "show_im_name", False))
        ...
        self.__checkbutton_embed_preedit_text.set_active(
            self.__config_get_value("general", "embed_preedit_text", True))
        ...
        self.__checkbutton_use_sys_layout.set_active(
            self.__config_get_value("general", "use_system_keyboard_layout", True))
        ...
        self.__checkbutton_use_global_engine.set_active(
            self.__config_get_value("general", "use_global_engine", False))
        ...
        engine_names = self.__config_get_value("general", "preload_engines", [])
    ...
    def __treeview_notify_cb(self, treeview, property):
        ...
        if property.name == "engines":
            ...
            self.__config_set_list("general", "preload_engines", engine_names, "s")
    ...
    def __shortcut_button_clicked_cb(self, button, name, section, _name, entry):
        ...
        if id != Gtk.ResponseType.OK:
            return
        self.__config_set_list(section, _name, shortcuts, "s")
        text = "; ".join(shortcuts)
        entry.set_text(text)
    ...
    def __item_preload_column_toggled_cb(self, cell, path_str, model):
        ...
        if data[DATA_PRELOAD]:
            if engine not in self.__preload_engines:
                ...
                self.__config_set_list("general", "preload_engines", list(self.__preload_engines), "s")
        else:
