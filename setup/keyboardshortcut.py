...
class KeyboardShortcutSelection(Gtk.VBox):
    ...
    def __init_ui(self):
        ...
        # shortcuts view
        self.__shortcut_view = Gtk.TreeView.new_with_model(Gtk.ListStore(str))
        ...
        self.__modifier_buttons.append(("Control",  Gtk.CheckButton.new_with_mnemonic("_Control"),    Gdk.ModifierType.CONTROL_MASK))
        self.__modifier_buttons.append(("Alt",      Gtk.CheckButton.new_with_mnemonic("A_lt"),        Gdk.ModifierType.MOD1_MASK))
        self.__modifier_buttons.append(("Shift",    Gtk.CheckButton.new_with_mnemonic("_Shift"),      Gdk.ModifierType.SHIFT_MASK))
        self.__modifier_buttons.append(("Meta",     Gtk.CheckButton.new_with_mnemonic("_Meta"),       Gdk.ModifierType.META_MASK))
        self.__modifier_buttons.append(("Super",    Gtk.CheckButton.new_with_mnemonic("S_uper"),      Gdk.ModifierType.SUPER_MASK))
        self.__modifier_buttons.append(("Hyper",    Gtk.CheckButton.new_with_mnemonic("_Hyper"),      Gdk.ModifierType.HYPER_MASK))
        self.__modifier_buttons.append(("Capslock", Gtk.CheckButton.new_with_mnemonic("Capsloc_k"),   Gdk.ModifierType.LOCK_MASK))
        self.__modifier_buttons.append(("Release",  Gtk.CheckButton.new_with_mnemonic("_Release"),    Gdk.ModifierType.RELEASE_MASK))
        ...
        hbox.pack_start(self.__add_button, False, False, 0)
        # apply button
        ...
        hbox.pack_start(self.__apply_button, False, False, 0)
        # delete button
        ...
        hbox.pack_start(self.__delete_button, False, False, 0)
        self.pack_start(hbox, False, True, 4)
    ...
    def __get_selected_shortcut(self):
        ...
        if path == None:
            return None
        return model.get(model.get_iter(path), 0)[0]

    def __set_selected_shortcut(self, shortcut):
        model = self.__shortcut_view.get_model()
        path, column = self.__shortcut_view.get_cursor()
        model.set_value(model.get_iter(path), 0, shortcut)
...
class KeyboardShortcutSelectionDialog(Gtk.Dialog):
    def __init__(self, title = None, parent = None, flags = 0, buttons = None):
        ...
        self.get_content_area().pack_start(self.__selection_view, False, False, 0)
