...
void new_connection_cb (IBusServer *server, IBusConnection *connection, gpointer user_data)
{
	...
	g_signal_connect (connection, "destroy", connection_destroy_cb, 0);
}
...
int main()
{
	...
	server = ibus_server_listen ("unix:abstract=/tmp/1234567");
	g_signal_connect (server, "new-connection", new_connection_cb, 0);
